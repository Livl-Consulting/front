...
import { addClientPayment } from "@/lib/actions/add-client-payment.action";

export const ClientPaymentsForm: FC<{
  totalDue: number;
  saleOrderId: number;
}> = ({ totalDue, saleOrderId }) => {
  const [state, action] = useActionState(addClientPayment, { success: false });
  const [date, setDate] = React.useState<Date | undefined>(undefined);
  const formattedDate = date ? format(date, 'yyyy-MM-dd') : '';

  return (
      <form action={action} className="flex flex-col gap-6">
          <Input type="hidden" name="orderId" value={saleOrderId}/>
